...
class QuestionAnswerTest(unittest.TestCase):
	...
	def test_answeredQuestion(self):
		...
		a_key=a4.put()
		########################
		ret=insertQuestionAnswered(users.get_current_user(),q_key,a_key)
		self.assertEquals('S',ret)
		ret=insertQuestionAnswered(users.get_current_user(),q_key,a_key)
		self.assertEquals('R',ret)
		query=AnsweredQuestion.query().fetch()
		self.assertEquals(1,len(query))
		ret=update_or_Insert_QuestionTestModule(str(q_key.id()),str(a_key.id()),users.get_current_user(),1.0)
		self.assertEquals('S',ret)
		update_or_Insert_QuestionTestModule(str(q_key.id()),str(a_key.id()),users.get_current_user(),1.0)
		query=AnsweredQuestion.query().fetch()
